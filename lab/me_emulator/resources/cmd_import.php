...
class cmd_import
{
  ...
  */
  ...
  function read_lines($lines)
  {
    ...
    if (!empty($lines->ignore))
    {
      foreach ($lines->ignore as $ignore)
      {
        ...
        {
          // $regexp is a SimpleXMLElement Object
          $regexp_str = trim((string)$regexp);
          if (!empty($regexp_str))
          {
            $ignore_parsers['regexp'][] = $regexp_str;
          }
        }
  ...
  function read_line($line)
  {
    ...
    foreach ($line->regexp as $regexp)
    {
      ...
      if (!empty($regexp_str))
      {
        ...
        {
          $parser['mregexp'][] = $regexp_str;
        }
        else
        {
          $parser['regexp'][] = $regexp_str;
        }
      }
    }
    ...
    {
      // $mregexp is a SimpleXMLElement Object
      $mregexp_str = trim((string)$mregexp);
      if (!empty($mregexp_str))
      {
        $parser['mregexp'][] = $mregexp_str;
      }
    }
  ...
  function parse($buffer, &$objects)
  {
    ...
    while ($line !== false)
    {
      ...
      if ($is_in_section)
      {
        ...
        {
          $ref_line = $line;
          echo ("PARSE LINE IN SECTION:$line REF LINE:$ref_line\n");
  ...
  {
    debug_dump($line, "ELEMENT TO PARSE\n");
    $var_pattern = '/\(\?\<(\w+)\>/';
    $found = false;
      ...
      foreach ($parser['line'] as $line_parser)
      {
	    ...
	    echo ("RULE TYPE: $rule\n");
            if ($rule === 'array_name')
            {
              ...
              echo ("RULE ARRAY NAME: $array_name\n");
	    }
            else if ($rule === 'regexp')
            {
              $found = false;
              ...
              {
		echo ("RULE REGEXP: $regexp\n");
                if (regexp_match("Object template {$this->object_name}", $regexp, $line, $matches) > 0)
                {
                  $found = true;
		  echo("LINE: ".$line." MATCHES REGEXP: ".$regexp."\n");
	      ...
	      foreach ($rule_content as $regexp)
              {
                ...
                $found = true;
                echo("LINE: ".$line." MATCHES MREGEXP: ".$regexp."\n");
                // the line matches, get the variables (if no variables do nothing)
                if (regexp_match_all("Object template {$this->object_name}", $var_pattern, $regexp, $vars) > 0)
                {
                  foreach ($vars[1] as $var_name)
    ...
    if (!$found)
    {
      ...
      if (!empty($parser['ignore']))
      {
        foreach ($parser['ignore'] as $ignore_line)
        {
          ...
          {
	    echo("CHECK $line REGEXP $regexp\n");
            if (regexp_match("Object template {$this->object_name}", $regexp, $line, $matches) > 0)
            {
	      echo ("IGNORE LINE $line   REGEXP: $regexp\n");
